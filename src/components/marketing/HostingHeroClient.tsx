...
/** Hosting hero: full-width band, title + subtitle, breadcrumbs below (no kicker). */
export default function HostingHeroClient({ tint, title, subtitle, crumbPath }: Props) {
  const lang = usePathLang()
  const pathname = usePathname()
  const pathForCrumbs =
    pathname && pathname !== '/' && pathname.includes('/hosting') ? pathname : crumbPath
