...
/**
 * Services hero: full-width band, title + subtitle, breadcrumbs below subtitle (no kicker).
 */
export default function ServiceHeroClient({ tint, title, subtitle, crumbPath }: Props) {
  const lang = usePathLang()
  const pathname = usePathname()
  const pathForCrumbs =
    pathname && pathname !== '/' && pathname.includes('/services/') ? pathname : crumbPath
