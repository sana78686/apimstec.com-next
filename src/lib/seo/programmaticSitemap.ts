...
export async function getProgrammaticSitemapEntries(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    const [blogsRes, pagesRes] = await Promise.all([getBlogs('en'), getPages('en')])
    for (const b of normalizeBlogSlugs(blogsRes)) {
      entries.push({
    ...
    for (const slug of normalizePageSlugs(pagesRes)) {
      entries.push({
